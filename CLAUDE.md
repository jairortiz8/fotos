...
## Selfie (recortes) + descarga en alta resolución (2026-06-07)

Dos cosas que reportó Jair usando el sistema:

- **La búsqueda por selfie "no reconocía" rostros recortados — era la DETECCIÓN, no el matching.** Jair probaba recortando una cara de las propias fotos del evento y pegándola; no la reconocía. Diagnóstico (reproducido contra datos REALES de prod vía el proxy de Postgres + R2, evento `prueba` con 1055 embeddings): el **query de pgvector + threshold 0.55 estaban perfectos** (un embedding almacenado usado como query devuelve su foto al 100%). El problema era que **RetinaFace (det_10g) no detecta una cara recortada al ras**: necesita CONTEXTO alrededor y un tamaño mínimo. Las caras en esas fotos miden ~98px; un recorte ajustado (98–166px, la cara llenando el cuadro) devolvía **0 caras → `NoFaceDetectedError`** → el usuario veía "no detectamos una cara". Confirmado: escalar la imagen SIN borde sigue fallando (no es tamaño, es **margen**).
  - **Fix** (`apps/ml/face_recognition.py`): nuevo `_pad_for_detection` + fallback en `embedding_from_bytes` — si la detección directa no encuentra nada, reintenta con la imagen **escalada (lado menor ~640px) + borde 35% replicado**. Sólo corre cuando la detección directa falla → **un selfie normal no cambia de comportamiento**. En el fallback, si hay varias detecciones (borde espurio) elige la **cara más grande** en vez de tirar `MultipleFaces` (un recorte ajustado es de UNA cara). El flujo normal (varias caras reales → `MultipleFaces`) queda intacto.
  - **Verificado E2E contra prod** (modelo buffalo_l local + embeddings reales): recorte 98×98 sin margen pasa de `NoFace` → **match 86%**; +15% margen → 92%; cara pegada en lienzo blanco → 88%; imagen sin cara → sigue `NoFace` (sin falsos positivos); foto completa con 11 caras → sigue `MultipleFaces`. 3 tests nuevos (mock con detección por-llamada).
  - **Nota de producto**: Jair recortaba para *probar*; el flujo real es "el corredor sube SU selfie de frente" (que siempre detectó bien). El fix igual hace robusta la feature ante recortes/pegados. Caso aún no cubierto: cara MUY chica pegada en un lienzo ENORME (el upscale no la agranda porque el lienzo ya es grande) — improbable en uso real; si aparece, subir `det_size` o multiescala.

- **El botón "Descargar" del lightbox bajaba el PREVIEW (WebP 1200px con watermark), no el original.** El `href` apuntaba a `preview_url`. La descarga por ZIP (multi-select) SÍ bajaba originales — sólo el botón de UNA foto estaba mal.
  - **Fix**: nueva `PhotoDownloadView` (`/descargas/foto/<id>/`) que redirige a una **URL firmada del original** con `Content-Disposition: attachment` (fuerza descarga con el nombre original). `get_signed_url` acepta `download_filename`. Valida foto `approved` + evento no privado y `is_searchable()`. El "Ver tamaño completo" (ícono arriba) sigue mostrando el preview a propósito (ver en el navegador, no descargar). 3 tests nuevos.

**Deploy**: ambos fixes pusheados a `main` (Railway redeploya web/worker/beat). El de selfie aplica al **web** (donde corre `embedding_from_bytes` en la búsqueda). **Pendientes de Jair sin cambios**: límite de gasto en Railway, cambiar pass del admin, revocar el API token cuando no se use.

## Portada en R2 + redes del organizador + footer IG + descarga de a una (2026-06-07)

Ronda de feedback de Jair. Migración aditiva `events/0003` (4 ADD COLUMN, aplicada a prod ANTES de pushear para evitar carrera con worker/beat). Verificado en prod.

- **Portada del evento "se guardaba pero no se mostraba"** (causa REAL): `cover_image` era un `ImageField` con `FileSystemStorage` → iba al **disco LOCAL**, que en Railway es **efímero y NO se sirve** (no hay `static()` de media en prod). Se "guardaba" (la ruta en DB) pero el archivo no existía/no se servía. **Fix**: la portada ahora va a **R2** como todo lo demás. Campo nuevo `Event.cover_key` (el `ImageField` queda **dormido**, no se dropeó para no hacer migración destructiva — se puede sacar después). `process_event_cover` (en `imaging.py`) resize a 1400px → WebP q82 → R2 `event_covers/<slug>.webp`. Se sirve por **`EventCoverView`** (`/eventos/<slug>/portada.webp`) que streamea de R2 con `Cache-Control: public, max-age=7d`; `Event.cover_url()` devuelve esa URL **versionada con `?v=<updated_at>`** (cachea fuerte pero bustea al cambiar la portada). NO se usa URL firmada (expira + no cachea) ni se tocan permisos del bucket (sigue privado). El `EventForm` usa un campo `cover` (no-modelo) que la vista procesa en `form_valid` (create + update). Se muestra en la card de la home, de fondo en el hero del evento (atenuada), y en el OG/JSON-LD. **Pendiente de Jair**: subir una portada desde el dashboard para verla (el pipeline está testeado; sin portada la vista da 404 limpio).

- **Redes del organizador por evento**: campos `organizer_name` / `organizer_instagram` / `organizer_facebook` en `Event`, editables en el form del dashboard. `instagram_url()`/`facebook_url()` normalizan usuario (`@x`) o link completo. Se muestran en el hero del evento (nombre + íconos IG/FB) y el `organizer_name` va al JSON-LD `organizer`.

- **Footer: Instagram del sitio** (`@findyourfoto`) configurable via env `SITE_INSTAGRAM` (default `findyourfoto`, sin `@`). Context processor expone `site_instagram` + `site_instagram_url`. Se respeta la regla de no-hardcodear (igual que `site_name`).

- **Descarga: SOLO de a una foto, y como ARCHIVO** (no ZIP, no "webpage"). Jair: el ZIP/selección múltiple confunde. **Se sacó del frontend** la selección múltiple (checkbox en las cards, pill flotante, bottom sheet, el JS `photoSelection`/`startDownload`/`pollZip`). La descarga es por foto desde el lightbox. **`PhotoDownloadView` ahora PROXYea el original same-origin** con `Content-Disposition: attachment` + el `<a download>` (antes redirigía a una URL firmada de R2 → en iOS/Safari la imagen se **abría como página** en vez de bajar). Same-origin + attachment baja al carrete/Archivos. Rate limit nuevo `200/h` por IP (generoso para un corredor, frena scrapeo por el proxy). El backend de ZIP (`CreateZipDownloadView`, `generate_zip`, modelo `ZipDownload`, cron de cleanup) queda **dormido** (sin borrar, sin migración destructiva, sus tests siguen pasando); `_download_sheet.html` quedó huérfano. Se puede limpiar del todo más adelante si querés.

- **Verificación**: 396 tests (20 nuevos/actualizados), mypy/ruff/black limpios, CSS de Tailwind lo recompila el Docker en build (escanea templates). Navegador local: organizador + footer IG + galería sin ZIP + lightbox con `download` (sin errores de Alpine). **Prod**: descarga `200 + attachment + nombre original` (`6K2A8244.jpg`), footer IG presente, galería sin UI de ZIP, portada da 404 hasta que se suba una. El backend de ZIP sigue desplegado pero sin entrada desde la UI.

## Hora de disparo + orden + duplicados + flujo 600 + términos/contacto/FAQ (2026-06-07)

Ronda de feedback de Jair. 2 migraciones aditivas (`events`… no, `photos/0003` content_hash) aplicadas a prod ANTES de pushear. Verificado en prod.

- **La hora "salía rara" — NO era timezone, era que se leía el tag EXIF equivocado.** `TIME_ZONE` ya es `America/El_Salvador` y el web/worker la usan bien (verificado: prod renderiza la hora local correcta). El bug real: `_exif_to_dict` solo leía el **IFD principal** (`img.getexif()`), que tiene `DateTime` = hora en que se **EXPORTÓ/guardó** el archivo. La hora del **DISPARO** (`DateTimeOriginal`) vive en el **sub-IFD de EXIF** (`raw.get_ifd(ExifTags.IFD.Exif)`), que no se leía → la hora caía a `DateTime`. Ej. real (foto 162): EXIF `DateTime`=16 Mar 20:51 (export) vs `DateTimeOriginal`=14 Mar 05:51 (la carrera, de madrugada). **Fix**: leer el sub-IFD y mergearlo; `_parse_capture_time` prioriza DateTimeOriginal>DateTimeDigitized>DateTime y lo ancla a `get_default_timezone()` (El Salvador), **ignorando `OffsetTimeOriginal`** porque las cámaras lo traen mal (esa foto tenía `-12:00`). Bonus: ahora también se extraen ISO/FNumber/ExposureTime/FocalLength/LensModel (vivían en el sub-IFD → salían vacíos en el drawer). **Sólo afecta uploads NUEVOS**; las fotos viejas conservan su hora hasta reprocesarse (reprocesar resetea a `pending_review`, así que es opcional/manual).
- **Orden cronológico**: galería pública (`apps/events/views`) + cola de aprobación (`apps/dashboard/views/photos`) ahora ordenan por `capture_time, created_at` ASC (la primera tomada primero). Las fotos sin `capture_time` van al final (Postgres: NULLS LAST en ASC). La búsqueda por dorsal ya estaba así.
- **Duplicados**: campo `Photo.content_hash` (sha256 del contenido, `db_index`, migración `photos/0003`). En `UploadView`, antes de crear la Photo, se hashea el archivo y si ya existe una foto con ese hash en el evento (no borrada) → **409 `duplicate`**. El portal lo marca **"Repetida"** (gris, ícono de copia, NO error, NO reintento). Dedup por evento (la misma foto en otro evento se permite). Hay una ventana de carrera mínima en uploads concurrentes idénticos (chequeo por query, no constraint) — aceptable para el caso real (re-subir lotes que se solapan).
- **Flujo de 600 fotos**: el éxito para el fotógrafo ahora es **"Subida"** (la foto LLEGÓ al server) en vez de esperar al worker. Antes el estado iba a "Procesando" y el polling se rendía a los 40 ciclos (~2 min); con 600 fotos el worker tarda mucho más → quedaban **pegadas en "Procesando"** para siempre. Ahora: 2xx → "Subida" (verde, éxito); el polling sólo MEJORA el estado (trae el thumbnail / detecta `processing_failed`/`rejected`) y si el worker tarda, la foto queda "Subida" verde (nunca pegada). Resumen del header suma "repetidas". La cola de 4 concurrentes + retry 429 ya estaban.
- **Legal**: (1) Términos — se sacó el link "Borrar mis datos" del cuerpo + se agregó cláusula **"Consentimiento del participante"** (aceptó ser fotografiado y la publicación). El banner global de privacidad (en `base.html`, todas las páginas) SIGUE teniendo el link de borrado — es un aviso de privacidad, no "los términos"; si Jair lo quiere fuera del banner también, avisar. (2) **Contacto = Instagram** (`@findyourfoto` vía `site_instagram`); se sacó el CTA de borrado + los placeholders de WhatsApp/email. (3) Nueva página **FAQs** (`/preguntas/`, `core:faq`) con ~9 preguntas + link en el footer.
- **Verificación**: 404 tests (11 nuevos), ruff/black/mypy limpios. EXIF probado contra la foto real 162 (pasó de 16 Mar 20:51 → 14 Mar 05:51 + trajo ISO/lente). Portal local carga sin errores de Alpine tras el rediseño de estados. **Prod**: FAQ 200, contacto con @findyourfoto, términos con la cláusula, footer con "Preguntas". El EXIF/duplicados se confirman cuando Jair re-suba fotos (el código quedó verificado local + unit).
- **Pendiente opcional**: reprocesar las fotos viejas de prod para corregirles la hora (las re-pondría en `pending_review`). Y, si Jair quiere, sacar el link de borrado también del banner global.

## Privacidad recortada + subida a prueba de errores (2026-06-07, parte 2)

- **"Borrar mis datos" fuera del banner + secciones de privacidad** (a pedido de Jair, sigue la línea de sacarlo de los términos): quitado el link del **banner global** (`base.html`, queda solo "Más info") y eliminadas las secciones **"Cómo borrar tus datos"** y **"Protección de menores"** de `privacy_policy.html`. La de menores describía el blur, que está **APAGADO en prod** → el texto era inexacto. La feature `delete_my_data` (view/URL) sigue existiendo; el **footer** todavía tiene "Borrar mis fotos" (Jair no lo pidió sacar — si quiere, se saca). Verificado en prod.

- **Subida a prueba de errores** (Jair: "la gracia es que no haya error, porque al fotógrafo le toca cazar por nombre la que falló y es tedioso"). Causas reales de falla identificadas + arregladas:
  - **Tamaño**: límite 15MB → **40MB** (`PHOTO_UPLOAD_MAX_MB`). Las fotos pro (Canon R 45MP) pesan 20-30MB y daban `file_too_large`. (Si prod tiene la env var seteada en 15, hay que subirla; si no, toma el default 40.)
  - **Reintento de TRANSITORIOS**: el cliente antes sólo reintentaba 429; los **cortes de red** (`xhr.onerror`) y los **5xx** quedaban en "Error" sin reintento → causa #1 de "algunas fallaron" en lotes grandes por WiFi inestable. Ahora `_retryTransient` reintenta red+5xx+429 con backoff (hasta 6, ~54s de ventana). Las **permanentes** (`_failPermanent`: formato/tamaño/límite/link) muestran el **motivo claro** ("Solo JPG", "Muy grande", "Límite alcanzado", "Link vencido") en vez de "Error" genérico — reintentar no las cambia.
  - **Botón "Reintentar las que fallaron"** (`retryFailed()`): re-encola TODAS las que quedaron en error de un toque, **sin buscar por nombre**. Aparece sólo si `counts.error > 0`.
  - Verificado en navegador: dropzone "Máximo 40 MB", contador "X con error", botón visible, motivos por foto, sin errores de Alpine. Strings nuevos de UI ("Error de red", "Muy grande", etc.) renderizan por fallback de Django (es=msgid); **catálogo i18n quedó con deuda menor** (no se corrió `makemessages` esta ronda — los `{% trans %}` nuevos no están en el `.po/.mo`, pero funcionan en español).
- **Nota de operación**: un deploy de Railway tardó **>13 min** esta sesión (el poll de verificación dio timeout a los 13' pero el cambio terminó saliendo). Los deploys no siempre son rápidos aunque el build esté cacheado.

## Portada en dashboard + regenerar link + datos demo (2026-06-07, parte 3)

- **Portada no se mostraba EN EL DASHBOARD**: la ronda anterior arreglé los templates PÚBLICOS (`home`/galería) para usar `event.cover_url` (R2), pero **me faltaron los del dashboard** (`event_detail.html`, `dashboard/home.html`) que seguían usando `event.cover_image.url` (ImageField viejo, disco efímero). Corregido. (El evento real `prueba` tiene `cover_key` seteado → la portada existe en R2; ahora se ve también en el dashboard.) Queda 1 uso de `cover_image` en `apps/events/admin.py` (Django admin de emergencia en `/admin/django/`) — inofensivo, no se tocó.
- **Links de fotógrafo "sólo se ven una vez"**: el token se guarda **hasheado** (sha256, regla de seguridad §3) → no se puede re-mostrar. La ruta `regenerate_link` existía pero **no había botón**. Agregado **"Regenerar"** en la lista de fotógrafos (`photographer_list.html`): form HTMX → modal `generate_link_success.html` con la URL nueva + QR + WhatsApp; se agregó `#modal-root` a esa página (sólo estaba en `event_detail`). **Decisión de Jair (preguntado)**: mantener hash puro (no guardar el token cifrado para re-verlo) → regenerar crea uno nuevo y revoca el anterior. Verificado E2E en navegador (login → lista → Regenerar → modal con link nuevo + QR). Test en `test_misc.py`.
- **Datos demo en prod**: identificados 3 eventos seed sintéticos (`maraton-antigua-2026`, `trail-ipala-2026`, `10k-san-salvador-2025`, del 2026-05-21, 0 caras reales, filenames `synth_*`/`DSC_*`). **Decisión de Jair (preguntado): NO borrarlos por ahora.** El evento real es `prueba` (178 fotos HEIC reales, 1055 embeddings, portada). No se borró nada.

## Hero (Pexels difuminada) + carpetas por fotógrafo + 'borrar mis fotos' fuera (2026-06-08)

- **Hero más llamativo (a pedido de Jair)**: el hero borroso/muddy original pasó a una **foto stock de carrera difuminada**. Jair eligió (preguntado) "foto stock de carreras pero difuminada". Bajada de **Pexels** (licencia libre, uso comercial, sin atribución) una foto de **pista roja + zapatillas** (sin caras → sin tema de consentimiento), blur gaussiano (radius 9) + saturación, `static/img/hero-race.webp`. Tratamiento: `.rf-hero-scrim` responsive (desktop oscurece izquierda y deja la pista vibrante a la derecha; mobile oscurece parejo para legibilidad) + brillo naranja de marca + zoom lento Ken Burns (respeta `prefers-reduced-motion`). **Aprendizaje**: 2 fotos previas fallaron — la 1ª (pelotón a contraluz) difuminada daba siluetas oscuras = volvía a la mancha; la de la pista (frontal, colorida) quedó vibrante. CSP de prod permite `style-src 'unsafe-inline'` (el `<style>` del hero anda). Prod hashea los estáticos (cache-bust automático). **Quick win**: placeholder de eventos sin portada (nombre del evento sobre degradé con brillo naranja) en la home + en las carpetas de fotógrafo.
- **'Borrar mis fotos' fuera del footer** (Jair: "rotundamente se tiene que quitar"): era el ÚLTIMO link visible a `delete_my_data` (ya fuera de términos/banner/privacidad/contacto). La vista/URL quedan dormidas (sin entrada UI). Si lo querés 404 del todo, hay que tocar los tests de `test_data_deletion.py` (pendiente, no se hizo).
- **Carpetas por fotógrafo (en cada evento, público + dashboard)** + **las generales por hora**:
  - `PhotographerLink.featured_image_key` (R2, webp) + migración aditiva `photographers/0002` (aplicada a prod). `featured_url()` → servida por `PhotographerCoverView` (`/u/cover/<id>.webp`, cache, URL versionada).
  - **El FOTÓGRAFO sube su 'imagen destacada' desde el portal** (decisión de Jair, preguntado): `/u/<token>/destacada/` (`PhotographerFeaturedUploadView`, csrf_exempt + rate 20/m, token-auth). Card nueva en `portal.html` (Alpine `featuredCover()`). `imaging.process_event_cover` refactorizado → `_process_cover_to_key` + `process_photographer_cover`.
  - **Público**: en `event_gallery.html`, toggle **'Todas (por hora)' | 'Por fotógrafo'**. Las carpetas = fotógrafos con fotos aprobadas (`?vista=fotografos`), cada uno con su imagen destacada (o placeholder de marca) + nombre + conteo; click → `?fotografo=<id>` = sus fotos cronológicas + breadcrumb 'Fotógrafos / Nombre'. `_photo_grid.html` preserva el filtro en el infinite-scroll. La galería sigue cronológica (hora de disparo).
  - **Dashboard ('ambos lados', decisión de Jair)**: thumbnail de la destacada en la lista de fotógrafos.
- **Bug encontrado al verificar**: 2 comentarios `{# #}` **MULTILÍNEA** se renderizaban como TEXTO (el bug conocido de Django): `event_gallery.html` ("La descarga es por foto…", **visible en prod desde la ronda del ZIP**) y `photographer_list.html` ("Regenerar"). Pasados a `{% comment %}`. **Recordatorio**: nunca usar `{# #}` multilínea.
- **Verificación**: 412 tests (8 nuevos), ruff/black/mypy limpios. Navegador local: carpetas + toggle + vista por fotógrafo + breadcrumb + card del portal (sin errores de Alpine). Hero desktop+mobile. Migración ya en prod.
