...
- **Pendiente Jair**: (1) poner un **límite de gasto** en Railway (Settings → Usage Limits) — ahora hay 3 servicios corriendo; (2) `SITE_NAME=find your foto` en la env var del web (sigue pendiente de Fase 7); (3) cambiar la pass temporal del admin; (4) **revocar el API token** de Railway cuando no haga falta.
